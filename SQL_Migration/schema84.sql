...
IF tg_op = 'INSERT' THEN
  IF new.pci_compliance = 'Yes' and new.first_questionnaire_date IS NOT NULL THEN
    INSERT INTO pci_compliance VALUES (NEXTVAL('pci_compliance_id_seq'), pcdtidsscan, smid, new.first_questionnaire_date);
    RETURN new;
  END IF;
  RETURN NULL;
END IF;
...
CREATE VIEW axia_compliant AS
    SELECT smsx.saq_merchant_id, smsx.datecomplete FROM saq_merchant_survey_xref smsx WHERE (smsx.acknowledgement_name IS NOT NULL);
...
CREATE TABLE bet (
    bet_code character varying(15) NOT NULL,
...
CREATE VIEW saq_eligible AS
    SELECT sm.id, sm.merchant_id, sm.merchant_email, sm.merchant_name, sm.password, sm.email_sent, scs.saq_type, scs.first_scan_date, scs.first_questionnaire_date, scs.scan_status, scs.questionnaire_status, scs.pci_compliance, scs.creation_date, scs.sua, mp.saq_completed_date, sm.billing_date, sm.next_billing_date, scs.quarterly_scan_fee, mp.compliance_fee, mp.insurance_fee FROM (((((((saq_merchant sm LEFT JOIN merchant m ON ((((sm.merchant_id)::text = (m.merchant_id)::text) AND (m.active = 1)))) LEFT JOIN merchant_pci mp ON (((sm.merchant_id)::text = (mp.merchant_id)::text))) LEFT JOIN saq_control_scan scs ON ((substr((sm.merchant_id)::text, 5, 12) = substr((scs.merchant_id)::text, 5, 12)))) LEFT JOIN merchant_cancellation mc ON ((((sm.merchant_id)::text = (mc.merchant_id)::text) AND (mc.date_completed IS NOT NULL)))) LEFT JOIN timeline_entries tes ON ((((sm.merchant_id)::text = (tes.merchant_id)::text) AND (((tes.timeline_item)::text = 'SUB'::text) AND (tes.timeline_date_completed < '2010-08-01'::date))))) LEFT JOIN timeline_entries tei ON ((((sm.merchant_id)::text = (tei.merchant_id)::text) AND ((tei.timeline_item)::text = 'INS'::text)))) LEFT JOIN bankcard b ON (((sm.merchant_id)::text = (b.merchant_id)::text))) WHERE ((((((sm.merchant_email)::text ~~ '%@%'::text) AND (m.merchant_id IS NOT NULL)) AND ((tei.merchant_id IS NOT NULL) OR (tes.merchant_id IS NOT NULL))) AND (b.merchant_id IS NOT NULL)) AND (mc.merchant_id IS NULL));
...
CREATE VIEW compliant_merchants_cs AS
    SELECT se.id, se.merchant_id, se.merchant_email, se.merchant_name, se.password, se.email_sent, se.saq_type, se.first_scan_date, se.first_questionnaire_date, se.scan_status, se.questionnaire_status, se.pci_compliance, se.creation_date, se.sua, se.saq_completed_date, se.billing_date, se.next_billing_date, se.quarterly_scan_fee, se.compliance_fee, se.insurance_fee FROM saq_eligible se WHERE ((se.pci_compliance)::text = 'Yes'::text);
...
CREATE VIEW foo AS
    SELECT tnc.dba, mtb.merchant_id, mtb.billing_date FROM (merchants_to_bill mtb LEFT JOIN tsys_non_compliances tnc ON ((mtb.merchant_id = (tnc.merchant_id)::text)));
...
CREATE VIEW non_compliant_merchants AS
    SELECT se.id, se.merchant_id, se.merchant_email, se.merchant_name, se.password, se.email_sent, se.saq_type, se.first_scan_date, se.first_questionnaire_date, se.scan_status, se.questionnaire_status, se.pci_compliance, se.creation_date, se.sua, se.saq_completed_date, se.billing_date, se.next_billing_date, se.quarterly_scan_fee, se.compliance_fee, se.insurance_fee FROM (saq_eligible se LEFT JOIN compliant_merchants cm ON (((se.merchant_id)::text = (cm.merchant_id)::text))) WHERE (cm.merchant_id IS NULL);
...
CREATE VIEW non_compliant_merchants_cs AS
    SELECT se.id, se.merchant_id, se.merchant_email, se.merchant_name, se.password, se.email_sent, se.saq_type, se.first_scan_date, se.first_questionnaire_date, se.scan_status, se.questionnaire_status, se.pci_compliance, se.creation_date, se.sua, se.saq_completed_date, se.billing_date, se.next_billing_date, se.quarterly_scan_fee, se.compliance_fee, se.insurance_fee FROM saq_eligible se WHERE ((se.pci_compliance)::text = 'No'::text);
...
CREATE TABLE onlineapp_coversheets (
    id integer NOT NULL,
    onlineapp_application_id integer NOT NULL,
    user_id integer NOT NULL,
...
CREATE VIEW pci_reminders AS
    SELECT sm.id, sm.merchant_id, (SELECT (max(pc.date_complete) + '1 year'::interval) FROM pci_compliance pc WHERE ((pc.saq_merchant_id = sm.id) AND (pc.pci_compliance_date_type_id = ANY (ARRAY[1, 2, 4])))) AS saq_deadline, (SELECT (max(pc.date_complete) + '3 mons'::interval) FROM pci_compliance pc WHERE ((pc.saq_merchant_id = sm.id) AND (pc.pci_compliance_date_type_id = ANY (ARRAY[3, 5])))) AS scan_deadline, (SELECT CASE WHEN ((max(pc.date_complete) + '1 year'::interval) < now()) THEN 'SAQ EXPIRED'::text WHEN ((max(pc.date_complete) + '1 year'::interval) < (now() + '1 mon'::interval)) THEN 'SEND SAQ REMINDER'::text ELSE (date_part('day'::text, (((max(pc.date_complete) + '1 year'::interval))::timestamp with time zone - now())) || ' Days Left'::text) END AS "case" FROM pci_compliance pc WHERE ((pc.saq_merchant_id = sm.id) AND (pc.pci_compliance_date_type_id = ANY (ARRAY[1, 2, 4])))) AS saq_status, (SELECT CASE WHEN ((max(pc.date_complete) + '3 mons'::interval) < now()) THEN 'SCAN EXPIRED'::text WHEN ((max(pc.date_complete) + '3 mons'::interval) < (now() + '20 days'::interval)) THEN 'SEND SCAN REMINDER'::text ELSE (date_part('day'::text, (((max(pc.date_complete) + '3 mons'::interval))::timestamp with time zone - now())) || ' Days Left'::text) END AS "case" FROM pci_compliance pc WHERE ((pc.saq_merchant_id = sm.id) AND (pc.pci_compliance_date_type_id = ANY (ARRAY[3, 5])))) AS scan_status, (SELECT pc.pci_compliance_date_type_id FROM pci_compliance pc WHERE ((pc.pci_compliance_date_type_id = ANY (ARRAY[1, 2, 4])) AND (pc.saq_merchant_id = sm.id)) ORDER BY pc.date_complete DESC LIMIT 1) AS saq_date_type_id, (SELECT pc.pci_compliance_date_type_id FROM pci_compliance pc WHERE ((pc.pci_compliance_date_type_id = ANY (ARRAY[3, 5])) AND (pc.saq_merchant_id = sm.id)) ORDER BY pc.date_complete DESC LIMIT 1) AS scan_date_type_id FROM ((saq_merchant sm LEFT JOIN pci_compliance pc ON ((sm.id = pc.saq_merchant_id))) JOIN pci_compliance_date_type pcdt ON ((pc.pci_compliance_date_type_id = pcdt.id))) GROUP BY sm.id, sm.merchant_id ORDER BY sm.id DESC;
...
CREATE VIEW reminder AS
    SELECT DISTINCT saq_merchant_pci_email_sent.saq_merchant_id, count(saq_merchant_pci_email_sent.saq_merchant_id) AS count FROM saq_merchant_pci_email_sent WHERE (saq_merchant_pci_email_sent.date_sent < '2011-09-01 00:00:00'::timestamp without time zone) GROUP BY saq_merchant_pci_email_sent.saq_merchant_id HAVING (count(saq_merchant_pci_email_sent.saq_merchant_id) < 2);
...
CREATE TABLE tickler_leads (
    ...
    notes character varying(512) DEFAULT NULL::character varying,
...
--
-- Name: onlineapp_pkey; Type: INDEX; Schema: public; Owner: axia; Tablespace: 
--

CREATE UNIQUE INDEX onlineapp_pkey ON onlineapp_applications USING btree (id);
...
CREATE TRIGGER pci_billing_audit_ins
    AFTER INSERT OR DELETE OR UPDATE ON pci_billing
    FOR EACH ROW
...
CREATE TRIGGER pci_compliance_status_audit_ins
    AFTER INSERT OR DELETE ON pci_compliance
    FOR EACH ROW
...
CREATE TRIGGER perform_update_pc_ax
    AFTER UPDATE ON saq_merchant_survey_xref
    FOR EACH ROW
...
CREATE TRIGGER perform_update_pc_cs_del
    AFTER DELETE ON saq_control_scan
    FOR EACH ROW
...
CREATE TRIGGER perform_update_pc_cs_saq_ins
    AFTER INSERT ON saq_control_scan
    FOR EACH ROW
...
CREATE TRIGGER perform_update_pc_cs_up
    AFTER UPDATE ON saq_control_scan
    FOR EACH ROW
